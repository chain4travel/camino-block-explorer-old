<template>
  <div>
      ...
      <q-card-section class="container">
        <q-list bordered separator>
          <DetailField
            ...
            field="Number"
            :value="content.blockNumber"
            type="string"
          />
          <DetailField
            icon="mdi-help-circle-outline"
            field="Parent Hash"
            :value="content.parentHash"
            type="string"
            :details-link="
              content.parentBlockNumber
                ? getBlockDetailsPath(type, content.parentBlockNumber)
                : undefined
            "
            :allow-copy="true"
          />
          ...
          <DetailField
            ...
            :value="content.transactionCount"
            type="string"
          />
          <DetailField
            icon="mdi-help-circle-outline"
            field="Extra Data"
            :value="content.additionalInformation.extraData"
            type="hexdata"
          />
        </q-list>
      </q-card-section>
    ...
    <CTransactionList
      ...
      class="q-mt-md"
      v-if="content.transactions && content.transactions.length"
  ...
  setup() {
    ...
    return {
      ...
      getTransactionDetailsLink: (hash: string): string => {
        return `${getTransactionDetailsPath(ChainType.C_CHAIN, hash)}?back=${
          route.fullPath
        }`;
      },
    };
