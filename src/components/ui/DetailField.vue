<template>
  <q-item class="table-item">
    <div class="row m-auto--md elements">
      <div
        :class="getTitleClass($q.screen.gt.xs, detailsLink, allowCopy, value)"
      >
        <div class="row">
          <q-icon
            class="col-2 gt-sm grey-color"
            v-if="icon"
            size="xs"
            :name="icon"
          >
          ...
          <span
            :class="'col overflow-handle ' + ($q.screen.lt.md ? '' : 'q-pl-md')"
          >
            {{ field }}
          </span>
        </div>
        ...
        <div v-else-if="type == 'timestamp'">
          <div class="row">
            <div class="q-pr-md col-auto">
              <q-icon class="q-pr-sm" size="xs" name="mdi-clock-outline" />{{
      ...
      <div
        ...
        :class="
          $q.screen.gt.xs
            ? detailsLink
              ? 'col-md-3 col-4 text-right m-auto--md q-gutter-y-xs'
              : 'col-md-2 col text-right m-auto'
            : 'col-6 q-gutter-y-xs m-auto item-buttons'
        "
      >
        <q-btn
          v-if="detailsLink"
...
function getTitleClass(
  ...
  } else return 'col-md-4 col-12 text-bold flex m-auto--md';
}

function getContentClass(size: boolean | undefined) {
  if (!size) return 'col-md col-12 q-mt-xs flex items-center item-content';
  else return 'col-md col-8 flex items-center';
}
